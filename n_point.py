...
#Define list of sigma-values for self-adaptation
sigma_vals = []
for i in sigma_vals:
    sigma = np.random.normal(0, gaussian_mutation_sd)
...
def mutate_selfadaptation(individual, tao):
    # update sigma
    #sigma = sigma_vals[]
    sigma_prime = sigma * math.e**(tao * np.random.normal(0,1))
    print(sigma_prime)

    # mutate weights
    for i in range(len(individual)):
        if random.random() < mutation_strength:
            individual[i] += sigma_prime*np.random.normal(0,1)  # You can adjust the mutation range
    return individual
...
while Gen < maxGens:
    ...
    for i in range(0,len(new_kids),2):
        baby1, baby2 = npoint_recombination(parents[random.randint(0, 99)], parents[random.randint(0, 99)], 2)
        new_kids[i] = baby1
        new_kids[i+1] = baby2
